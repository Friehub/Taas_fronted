 ...
 * Rebuilt to match the provided mockup with Resend integration.
 */
export const Footer: React.FC = () => {
  ...
  return (
    <footer className="w-full bg-background border-t border-white/[0.03] pt-24 pb-12 px-8">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 lg:gap-8">
        
        {/* Column 1: Brand & Identity */}
        <div className="space-y-8">
          <div className="flex items-center space-x-2">
            <div className="h-6 w-6 bg-primary rounded-sm transition-transform group-hover:scale-110" />
            <span className="text-xl font-display font-bold text-foreground">Friehub</span>
          </div>
          <p className="text-secondary/40 text-sm leading-relaxed max-w-xs">
            The high-integrity truth network built for the future of decentralized resource markets and prediction veracity.
          </p>
          <div className="flex items-center space-x-6">
             <Link href="https://x.com/friehub" className="text-secondary/30 hover:text-primary transition-colors">
             ...
             <Link href="https://github.com/Friehub" className="text-secondary/30 hover:text-primary transition-colors">
        ...
        <div className="space-y-6">
          ...
          <ul className="space-y-4">
            {["Ecosystem", "Governance", "Tokenomics", "Security"].map((item) => (
              <li key={item}>
                <Link href="#" className="text-sm text-secondary/40 hover:text-primary transition-colors">{item}</Link>
              </li>
            ))}
          </ul>
        </div>
          ...
          <form onSubmit={handleSubscribe} className="space-y-4">
             <div className="relative group">
                <input 
                  ...
                  required
                  className="w-full bg-background border border-white/5 px-4 py-3 text-sm text-foreground focus:outline-none focus:border-primary/30 transition-all placeholder:text-secondary/20"
                />
                <button 
                  type="submit"
                  disabled={status === "loading"}
                  className="absolute right-1 top-1 bottom-1 px-4 bg-primary text-background font-bold uppercase tracking-widest text-[10px] hover:scale-[1.02] transition-all disabled:opacity-50"
                >
                  {status === "loading" ? "..." : "→"}
                </button>
             </div>
             {status === "success" && <p className="text-[10px] text-primary font-mono lowercase">verified_email_added_to_registry</p>}
      ...
      <div className="max-w-7xl mx-auto mt-24 pt-8 border-t border-white/[0.03] flex flex-col md:flex-row justify-between items-center gap-6">
        <span className="text-[10px] font-mono text-secondary/20 uppercase tracking-widest">
          © 2026 / TRIE-NETWORK_AG_10. ALL RIGHTS RESERVED.
        </span>
        <div className="flex items-center space-x-8">
           <Link href="#" className="text-[10px] font-mono text-secondary/20 hover:text-primary transition-colors uppercase tracking-widest">Privacy Policy</Link>
           <Link href="#" className="text-[10px] font-mono text-secondary/20 hover:text-primary transition-colors uppercase tracking-widest">Terms of Service</Link>
        </div>
      </div>
    </footer>
  );

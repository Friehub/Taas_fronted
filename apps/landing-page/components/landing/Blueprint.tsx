    ...
    return (
        <section className="py-60 bg-background relative overflow-hidden border-y border-border">
            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                <div className="mt-40 max-w-4xl mx-auto p-12 glass-mint border border-primary/10 rounded-3xl relative overflow-hidden group bg-background/50">
...
function BlueprintCard({ icon, title, desc, i, active }: { icon: any, title: string, desc: string, i: number, active?: boolean }) {
    return (
        <motion.div
            ...
            className={`p-12 h-full flex flex-col items-start bg-background hover:bg-muted/50 transition-all duration-500 group ${active ? 'border-t-2 border-primary' : ''}`}
        >
            <div className="p-4 rounded-lg bg-muted text-foreground/40 group-hover:text-primary group-hover:bg-primary/10 transition-all mb-8 shadow-sm">
                {icon}
            </div>

...
export function MembershipBadge({ email }: MembershipBadgeProps) {
    ...
    return (
        <motion.div
            ...
            className="w-full max-w-md mx-auto p-8 rounded-xl border border-white/10 bg-surface-low backdrop-blur-sm relative overflow-hidden group"
        >
            {/* Background Detail */}
            <div className="absolute top-0 right-0 p-4 opacity-5">
                <ComponentInstanceIcon width={120} height={120} className="text-foreground -rotate-12" />
            </div>
            ...
            <div className="relative z-10">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h4 className="text-sm font-black uppercase tracking-widest text-foreground">Attestation Confirmed</h4>
                        <p className="text-[10px] text-foreground/40 font-mono">{email}</p>
                    </div>
                </div>

                <div className="mb-8 p-4 bg-background/50 border border-white/5 rounded-sm font-mono text-[12px]">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <button
                        onClick={handleCopy}
                        className="h-12 border border-white/10 hover:bg-white/5 rounded-sm flex items-center justify-center gap-2 transition-all text-[10px] font-black uppercase tracking-widest"
                    >
                        {copied ? (
                            <>
                                <CheckIcon className="text-foreground" /> Copied
                            </>
                        ) : (
                            <>
                                <CopyIcon /> Copy Proof
